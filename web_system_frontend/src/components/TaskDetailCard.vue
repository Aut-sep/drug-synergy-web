  ...
  <el-card class="panel-card task-detail-card" shadow="never">
    ...
    <template v-if="detail">
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="训练组/实验名">{{ versionGroupLabel }}</el-descriptions-item>
        <el-descriptions-item label="参与模型">{{ detail.selected_models.join('、') || '-' }}</el-descriptions-item>
        <el-descriptions-item label="更新时间">{{ detail.updated_at }}</el-descriptions-item>
        <el-descriptions-item v-if="modelVersionPairs.length" label="各模型版本" :span="2">
          <div class="task-detail-card__tag-row">
      ...
      <div v-if="artifactItems.length" class="task-detail-card__section">
        <div class="task-detail-card__section-head task-detail-card__section-head--stacked">
          <div>
            <h3>训练产物</h3>
            <span class="page-caption">共 {{ artifactItems.length }} 个文件，总大小 {{ formatSize(artifactTotalSize) }}</span>
          </div>
          <el-button v-if="hasMoreArtifacts" size="small" text @click="artifactExpanded = !artifactExpanded">
            {{ artifactExpanded ? '收起' : '展开全部' }}
          </el-button>
        </div>
        <div :class="['task-detail-card__artifact-list', { 'task-detail-card__artifact-list--scroll': artifactExpanded && hasMoreArtifacts }]">
          <button
            v-for="artifact in visibleArtifactItems"
            :key="artifact.path"
            type="button"
            class="task-detail-card__artifact-item"
            @click="openEndpoint(artifact.download_endpoint)"
          >
            <strong class="task-detail-card__artifact-path">{{ artifact.path }}</strong>
            <span class="task-detail-card__artifact-size">{{ formatSize(artifact.size_bytes) }}</span>
          </button>
        </div>
        <div v-if="hasMoreArtifacts" class="task-detail-card__artifact-footer">
          <span class="page-caption">
            {{ artifactExpanded ? `已显示全部 ${artifactItems.length} 个文件` : `已显示前 ${visibleArtifactItems.length} 个，剩余 ${hiddenArtifactCount} 个` }}
          </span>
        </div>
      </div>
      ...
      <div v-if="resourceCards.length" class="task-detail-card__section">
        <div class="task-detail-card__section-head">
          <h3>资源监控摘要</h3>
          <span class="page-caption">按模型汇总关键资源指标。</span>
        </div>
        <div class="info-grid">
...
const maxVisibleArtifactCount = 6
const artifactExpanded = ref(false)

const summaryLabelMap: Record<string, string> = {
  version_id: '版本 ID',
  version_group_id: '训练组 ID',
  row_count: '结果行数',
  selected_models: '参与模型',
...
const artifactItems = computed(() => props.detail?.artifact_file_items ?? [])
const artifactTotalSize = computed(() =>
  artifactItems.value.reduce((total, item) => total + Number(item.size_bytes || 0), 0),
)
const hasMoreArtifacts = computed(() => artifactItems.value.length > maxVisibleArtifactCount)
const visibleArtifactItems = computed(() =>
  hasMoreArtifacts.value && !artifactExpanded.value
    ? artifactItems.value.slice(0, maxVisibleArtifactCount)
    : artifactItems.value,
)
const hiddenArtifactCount = computed(() => Math.max(artifactItems.value.length - visibleArtifactItems.value.length, 0))

watch(
  () => [props.detail?.id, props.detail?.artifact_file_items.length ?? 0],
  () => {
...
const runtimeHint = computed(() => {
  ...
  if (detail.local_status === 'waiting') {
    return {
      title: '训练任务正在排队或准备环境',
      description: '训练开始前通常会先完成数据导出、环境检查和服务调度，这段等待属于正常现象。',
      type: 'info' as const,
    }
  }
  ...
  if ((silentMinutes ?? 0) >= 5) {
    return {
      title: '训练任务长时间未刷新',
      description: `该任务已运行约 ${runningMinutes ?? '-'} 分钟，且最近 ${silentMinutes ?? '-'} 分钟没有状态更新。建议先查看日志末尾，确认任务是否仍在推进。`,
  ...
  return {
    title: '训练任务仍在执行',
    description: `训练属于长时任务，运行约 ${runningMinutes ?? '-'} 分钟时仍显示 running 并不异常。只要日志或更新时间仍在变化，通常说明任务还在继续。`,
    type: 'info' as const,
  }
})
...
.task-detail-card__section-head--stacked {
  align-items: flex-start;
}

.task-detail-card__toolbar,
.task-detail-card__tag-row {
  display: flex;
  gap: 8px;
  flex-wrap: wrap;
}
...
.task-detail-card__artifact-list--scroll {
  max-height: 360px;
  overflow: auto;
  padding-right: 4px;
}
...
.task-detail-card__artifact-path {
  word-break: break-all;
}

.task-detail-card__artifact-size {
  color: #5b6b82;
}

.task-detail-card__artifact-footer {
  display: flex;
