...
class ContainerDeploymentArtifactTests(unittest.TestCase):
    def test_web_container_artifacts_are_present_and_wired(self) -> None:
        ...
        training_text = training_dockerfile.read_text(encoding="utf-8")
        ...
        self.assertIn("pypi.tuna.tsinghua.edu.cn", backend_text)
        self.assertIn("npm run build", frontend_text)
        self.assertIn('"fastapi==0.115.14"', model_service_text)
        self.assertIn("pypi.tuna.tsinghua.edu.cn", model_service_text)
        self.assertIn("pypi.tuna.tsinghua.edu.cn", training_text)
