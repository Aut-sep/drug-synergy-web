...
parser.add_argument('--result-name', type=str, default='main_run', help='Result subdirectory name under results/')
parser.add_argument('--save-models', dest='save_models', action='store_true', help='Save best model checkpoints.')
parser.add_argument('--no-save-models', dest='save_models', action='store_false', help='Do not save model checkpoints; keep best model in memory only.')
...
def train(model, train_data_loader, loss_fn, optimizer, n_epochs, device, scheduler=None, model_save_path='model.pkl', save_models=True):
    max_acc = 0
    best_model = None
...
def test(test_data_loader, model, fold):
    path = os.path.join(result_score_dir, str(fold))
    test_probas_pred = []
    test_ground_truth = []
    with torch.no_grad():
...
result_root = os.path.join('results', args.result_name)
result_model_dir = os.path.join(result_root, 'model')
result_score_dir = os.path.join(result_root, 'score')
os.makedirs(result_model_dir, exist_ok=True)
os.makedirs(result_score_dir, exist_ok=True)

file_result = os.path.join(result_root, 'folds.txt')
...
for i in range(5):
    ...
    model_save_path = os.path.join(result_model_dir, f'fold{i}_model.pkl')
    ...
    best_model = train(model, train_data_loader, loss, optimizer, n_epochs, device, scheduler, model_save_path, args.save_models)
    if args.save_models:
        test_model = torch.load(model_save_path)
    else:
        test_model = best_model.to(device=device)
