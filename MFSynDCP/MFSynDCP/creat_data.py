...
# 创建数据集
def creat_data(datafile, cellfile, csv_path=None):
    # 读取细胞特征文件
    file2 = cellfile
    cell_features = []
...
if __name__ == "__main__":
    ...
    for fold in range(5):
        for split in ['train', 'test']:
            datafile = f'fold{fold}_{split}'
            csv_path = f'data/fold/fold{fold}/{split}.csv'   # ← 只改这里
            creat_data(datafile, cellfile, csv_path=csv_path)
